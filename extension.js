...
function activate(context) {
	let disposable = vscode.commands.registerCommand('react-setstate.setState', async function () {
		const inputText = await vscode.window.showInputBox({
			...
			prompt: "Enter the name of your state, and optionally, provide a default value separated by a comma.",
		  });

		  if (!inputText) {
            vscode.window.showErrorMessage("You need to type in something for this to work 😊.");
            return;
        }
	
		  if (inputText == null) return;

		  let [stateName, ...defaultValue] = inputText.split(",").map(item => item.trim());
        ...
        if (defaultValue !== undefined) {
            defaultValueText = (isBooleanOrNumber(defaultValue) ? defaultValue : `"${defaultValue}"`);
        }
...
function isBooleanOrNumber(value) {
    return (value.toLowerCase() === 'true' || value.toLowerCase() === 'false') || !isNaN(value);
}
